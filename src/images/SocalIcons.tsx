...
const getIcons = (icon: string, fill: string) => {
  switch (icon) {
    case "github":
      return (
        <svg data-name="Layer 1" viewBox="0 0 24 24" id="github">
          <path
    ...
    case "instagram":
      return (
        <svg data-name="Layer 1" viewBox="0 0 24 24" id="instagram">
          <path
...
const SocialIcons: FC<SocialIconsProps> = ({ name, fill }) => {
  const { theme, themeName, toggleTheme } = React.useContext(ThemeContext);
  return <div>{getIcons(name, fill)}</div>;
};
